...
RSpec.describe PolicyManager do
  ...
  describe "#change_allowed?" do
    let(:dependency) { stub_internal_dependency("foo") }

    it "returns false if `auto_merge: false` is the default behaviour and no override is provided" do
      remote_config = { "defaults" => { "auto_merge" => false } }
      policy_manager = PolicyManager.new(remote_config)
      expect(policy_manager.change_allowed?(dependency, :patch)).to eq(false)
    end

    it "returns true if `auto_merge: true` is the default behaviour and no override is provided" do
      remote_config = { "defaults" => { "auto_merge" => true } }
      policy_manager = PolicyManager.new(remote_config)
      expect(policy_manager.change_allowed?(dependency, :patch)).to eq(true)
    end

    it "returns false if `auto_merge: true` is the default behaviour but is overridden to `false` on the dependency" do
      remote_config = { "defaults" => { "auto_merge" => true }, "overrides" => [{ "dependency" => dependency, "auto_merge" => false }] }
      policy_manager = PolicyManager.new(remote_config)
      expect(policy_manager.change_allowed?(dependency, :patch)).to eq(false)
    end
  end
